#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Templar Bastion (Black Templars) Runbook (Word) from tools/bt_bastion_data.py + mc_bt_bastion_sim.py.
   PYTHONPATH=tools:src python3 tools/gen_bt_bastion_docx.py
"""
import os
...
def main():
    ...
    r = t.add_run("BLACK TEMPLARS — TEMPLAR BASTION RUNBOOK"); r.bold = True; r.font.size = Pt(20); r.font.color.rgb = STEEL
    ...
    doc.add_paragraph("Games/archetype via tools/mc_bt_bastion_sim.py. Guns = the reliable Interception-Strike "
                      "package (2 Repulsor Executioners, full hit re-rolls) onto the priority target; Melee = the "
                      "crit-5 Sword Brethren brick (Accept vow, no delivery risk). Durable hold-and-grind — "
                      "indicative; coin-flips are the matchups to practise.").italic = True
    try:
    ...
    if rows:
        ...
        doc.add_paragraph(f"Prevalence-weighted BT-Bastion win rate ≈ {wr:.0f}% — a strong, CONSISTENT hold-and-"
                          f"grind list (reliable removal + durability = few bad matchups). Capped by SPEED: loses "
                          f"to fast/kiting gun lists and true hordes; grinds everything that stands and fights.").italic = True
