#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Templar Bastion (Black Templars) analysis workbook (Excel) from bt_bastion_data.py + mc_bt_bastion_sim.py.
   PYTHONPATH=tools:src python3 tools/gen_bt_bastion_xlsx.py
"""
import os
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
from openpyxl.utils import get_column_letter
import bt_bastion_data as D
import doc_versions as V

OUT = V.out_path("bt-bastion-analysis")
HEAD = PatternFill("solid", fgColor="202028")   # BT black
...
VF = {"FAV": "C6EFCE", "EVEN": "C6EFCE", "COIN": "FFEB9C", "UNFA": "F4B084", "HARD": "F4B084"}


def vfill(v):
    for k in ("EVEN", "FAV", "COIN", "UNFA", "HARD"):
...
def sheet_list(wb):
    ...
    title(ws, 1, D.LIST_NAME, 4, 13)
    ws.cell(2, 1, "Detachments").font = BOLD; ws.merge_cells("B2:D2"); ws.cell(2, 2, D.DETACHMENTS).alignment = WRAP
    ...
    ws.merge_cells("A7:D7"); ws.cell(7, 1, D.IDENTITY).alignment = WRAP; ws.row_dimensions[7].height = 130
...
def sheet_matchups(wb):
    ...
    title(ws, 1, "TEMPLAR BASTION vs the LSO meta (BT's perspective)", 5)
    ...
    for m in D.MATCHUPS:
        ...
        ws.row_dimensions[r].height = 160; r += 1
    widths(ws, [16, 30, 10, 22, 80]); ws.freeze_panes = "A3"


def sheet_sim(wb):
    ws = wb.create_sheet("Sim Verification")
    title(ws, 1, "DATA-DRIVEN SIM — Templar Bastion vs the meta (from data/bsdata via wh.mathhammer)", 6)
    ws.merge_cells("A2:F2"); ws.cell(2, 1, "tools/mc_bt_bastion_sim.py. Guns = reliable Interception-Strike RepEx "
                  "package onto the priority target; Melee = the crit-5 Sword Brethren brick (Accept vow). No "
                  "delivery risk. Indicative — coin-flips are the practice matchups.").alignment = WRAP; ws.row_dimensions[2].height = 42
    hrow(ws, 3, ["Archetype", "Prev.", "Verdict", "Guns", "Melee", "BT win%"])
    r = 4
    try:
        import mc_bt_bastion_sim
        rows = mc_bt_bastion_sim.results(2000)
    ...
    for x in rows:
        ...
        ws.cell(r, 4, x["shoot"]).alignment = CTR; ws.cell(r, 5, x["melee"]).alignment = CTR
    ...
    if rows:
        ...
        ws.cell(r, 1, f"Prevalence-weighted ≈ {wr:.0f}% — strong consistent hold-and-grind; capped by speed (loses to fast/kiting guns + hordes).").font = BOLD
    widths(ws, [34, 7, 8, 8, 8, 9]); ws.freeze_panes = "A4"


def sheet_rules(wb):
    ws = wb.create_sheet("Tapestry & Mindset")
    title(ws, 1, "THE TAPESTRY — how Templar Bastion wins", 2)
    ...
    ws.cell(r, 1, D.MINDSET).alignment = WRAP; ws.row_dimensions[r].height = 100; r += 2
    ws.merge_cells(start_row=r, start_column=1, end_row=r, end_column=2)
...
def main():
    ...
    V.stamp_xlsx(wb, "bt-bastion-analysis")
    wb.save(OUT)
