#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Black Templars 'Send Help' FIXED analysis workbook (Excel) from tools/bt_data.py + mc_bt_sim.py.
   PYTHONPATH=tools:src python3 tools/gen_bt_xlsx.py
"""
import os
...
def sheet_list(wb):
    ...
    ws.merge_cells("A7:D7"); ws.cell(7, 1, D.IDENTITY).alignment = WRAP; ws.row_dimensions[7].height = 120
...
def sheet_matchups(wb):
    ...
    title(ws, 1, "FIXED BLACK TEMPLARS vs the LSO meta (BT's perspective)", 5)
    ...
    for m in D.MATCHUPS:
        ...
        ws.row_dimensions[r].height = 155; r += 1
    widths(ws, [16, 30, 10, 22, 80]); ws.freeze_panes = "A3"


def sheet_sim(wb):
    ws = wb.create_sheet("Sim Verification")
    title(ws, 1, "DATA-DRIVEN SIM — fixed BT vs the meta (output from data/bsdata via wh.mathhammer)", 6)
    ws.merge_cells("A2:F2"); ws.cell(2, 1, "tools/mc_bt_sim.py. Spike = the delivered Sword Brethren spike "
                  "(crit-on-5 Lethal Hits, S7 vs Monster/Vehicle) onto the priority target; Shoot = BT's thin "
                  "Oath-boosted shooting. Delivery-dependent (2 buses). Indicative — coin-flips are the practice "
                  "matchups.").alignment = WRAP; ws.row_dimensions[2].height = 42
    hrow(ws, 3, ["Archetype", "Prev.", "Verdict", "Spike", "Shoot", "BT win%"])
    r = 4
    try:
        import mc_bt_sim
    ...
    for x in rows:
        ...
        ws.cell(r, 4, x["spike"]).alignment = CTR; ws.cell(r, 5, x["shoot"]).alignment = CTR
    ...
    if rows:
        ...
        ws.cell(r, 1, f"Prevalence-weighted BT win rate ≈ {wr:.0f}% — swingy mid/low-tier melee-spike army; hardest = T'au (delivery race) + hordes + speed.").font = BOLD
    widths(ws, [34, 7, 8, 9, 9, 9]); ws.freeze_panes = "A4"


def sheet_rules(wb):
    ws = wb.create_sheet("Tapestry & Mindset")
    title(ws, 1, "THE TAPESTRY — how the fixed Black Templars win", 2)
