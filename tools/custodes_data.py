...
RULES = [
    ...
    ("CORRECTION — Assemblage of Might is NOT in this list", "Assemblage of Might (the mark -> +1-to-wound rule) belongs to the AURIC CHAMPIONS detachment, not Shield Host (faction pack: Auric Champions Detachment, confirmed by the errata). This list is Shield Host + Tharanatoi Hammerblow, so it does NOT get Assemblage — there is no +1-to-wound mark. Shield Host's detachment rule is MARTIAL MASTERY (below). [Earlier drafts wrongly modelled Assemblage on the bricks; removed.]"),
...
VERIFIED_PROFILES = [
    ...
    ("Blade Champion — Vaultswords (3 PROFILES)", "Behemor S7 AP-2 D2 A6 [PRECISION] | Hurricanis S5 AP-1 D1 A9 [SUSTAINED HITS 1] | Victus S6 AP-3 D3 A5 [DEVASTATING WOUNDS]", "pick the profile per target — Behemor for characters, Hurricanis for hordes, Victus for elites/armour. (Earlier drafts only modelled Behemor.)"),
