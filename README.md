...
# 2b. (optional) BCP hands-free login — only if you'll re-pull NEW events' army lists.
#     Lets the tools auto-login to BestCoastPairings (no token pasting). Skip on a box with no browser.
npm --prefix tools/bcp_login install
npx --prefix tools/bcp_login playwright install --with-deps chromium   # ~115MB Chromium + apt libs (needs sudo)
cp .env.bcp.example .env.bcp     # then put your BCP_EMAIL / BCP_PASSWORD in it (gitignored)
...
| `.env.bcp` | BCP login creds (`BCP_EMAIL`/`BCP_PASSWORD`); only needed to **re-pull a new event's** lists (this event's are committed). With the step-2b deps the tools auto-login via headless Chromium — no token pasting. See `data/bcp/README.md` |
| `tools/bcp_login/node_modules/`, `data/.bcp_token_cache.json` | the Playwright browser deps + the cached (~1h) BCP token — regenerated by step 2b / on first login |
